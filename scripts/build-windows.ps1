...
Write-Host ""
Write-Host "  Set Azure Artifact Signing env vars before a release build:"
Write-Host "    AZURE_ARTIFACT_SIGNING_ACCOUNT_NAME"
Write-Host "    AZURE_ARTIFACT_SIGNING_CERTIFICATE_PROFILE_NAME"
Write-Host "    AZURE_ARTIFACT_SIGNING_ENDPOINT"
Write-Host "    AZURE_TENANT_ID / AZURE_CLIENT_ID / AZURE_CLIENT_SECRET"
Write-Host "    WINDOWS_SIGN_TIMESTAMP_URL (optional; defaults to Artifact Signing TSA)"
Write-Host ""
Write-Host "  Install the supported Artifact Signing client tools on the Windows signer:"
Write-Host "    winget install -e --id Microsoft.Azure.ArtifactSigningClientTools"
Write-Host ""
Write-Host "  Optional overrides:"
Write-Host "    AZURE_ARTIFACT_SIGNING_METADATA_PATH"
Write-Host "    AZURE_ARTIFACT_SIGNING_DLIB_PATH"
Write-Host "    AZURE_ARTIFACT_SIGNING_EXCLUDE_CREDENTIALS"
Write-Host "    AZURE_ARTIFACT_SIGNING_ALLOW_NUGET_DLIB=1  # unsupported fallback"
Write-Host "    NUGET_EXE_PATH"
Write-Host ""
Write-Host "  To build everything:  powershell -File scripts\build-windows.ps1"
Write-Host "  To check the Azure certificate profile:"
Write-Host "    powershell -File scripts\check-artifact-signing-profile.ps1 -ResourceGroup <rg> -AccountName <account> -ProfileName <profile>"
