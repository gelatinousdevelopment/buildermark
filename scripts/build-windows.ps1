...
#   WINDOWS_SIGN_TIMESTAMP_URL  - RFC3161 timestamp URL (default: DigiCert)
...
function Get-SigningConfig() {
    ...
    if (-not $certFile -and -not $certThumbprint) {
        throw "Signing requires WINDOWS_SIGN_CERT_FILE or WINDOWS_SIGN_CERT_THUMBPRINT."
    }
...
function Sign-File($signToolPath, $signingConfig, $path) {
    ...
    if ($LASTEXITCODE -ne 0) { throw "signtool sign failed for $path (exit code $LASTEXITCODE)" }
}
