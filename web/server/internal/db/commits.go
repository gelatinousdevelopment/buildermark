...
func CountCommitsByProject(ctx context.Context, db *sql.DB, projectID string) (int, error) {
	var count int
	err := db.QueryRowContext(ctx, "SELECT COUNT(*) FROM commits WHERE project_id = ?", projectID).Scan(&count)
	if err != nil {
...
func GetCommitByHash(ctx context.Context, db *sql.DB, projectID, commitHash string) (*Commit, error) {
	var c Commit
	...
	).Scan(&c.ID, &c.ProjectID, &c.CommitHash, &c.Subject, &c.AuthorName, &c.AuthorEmail, &c.AuthoredAt,
		&c.DiffContent, &c.LinesTotal, &c.CharsTotal, &c.LinesFromAgent, &c.CharsFromAgent)
...
func OldestCommitByProject(ctx context.Context, db *sql.DB, projectID string) (*Commit, error) {
	var c Commit
	err := db.QueryRowContext(ctx,
		`SELECT id, project_id, commit_hash, subject, author_name, author_email, authored_at,
		        diff_content, lines_total, chars_total, lines_from_agent, chars_from_agent
...
func UpdateCommitCoverage(ctx context.Context, db *sql.DB, projectID, commitHash string, linesFromAgent, charsFromAgent int) error {
	_, err := db.ExecContext(ctx,
		`UPDATE commits SET lines_from_agent = ?, chars_from_agent = ? WHERE project_id = ? AND commit_hash = ?`,
		linesFromAgent, charsFromAgent, projectID, commitHash,
	)
	if err != nil {
