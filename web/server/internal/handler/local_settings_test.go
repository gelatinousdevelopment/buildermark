...
func TestGetLocalSettings(t *testing.T) {
	...
	if env.Data.HomePath != "/tmp/zrate-home" {
		t.Fatalf("homePath = %q, want %q", env.Data.HomePath, "/tmp/zrate-home")
	}
	...
	if got := env.Data.ConversationSearchPaths[0]; got.Agent != "claude" || got.Path != "/tmp/zrate-home/.claude" {
		t.Fatalf("first path = %+v, want agent=claude path=/tmp/zrate-home/.claude", got)
	}
	if got := env.Data.ConversationSearchPaths[1]; got.Agent != "codex" || got.Path != "/tmp/zrate-home/.codex" {
		t.Fatalf("second path = %+v, want agent=codex path=/tmp/zrate-home/.codex", got)
	}
}
