...
// UpdateProjectGitWorktreePaths sets the git_worktree_paths on a project.
func UpdateProjectGitWorktreePaths(ctx context.Context, db *sql.DB, projectID, paths string) error {
	_, err := db.ExecContext(ctx, "UPDATE projects SET git_worktree_paths = ? WHERE id = ?", paths, projectID)
	if err != nil {
		return fmt.Errorf("update project git_worktree_paths: %w", err)
	}
	return nil
}
